...
Stack.prototype.push = function(value) {
  if (this.storage && this.count() >= this.capacity) {
    return;
  }

  this.storage += `${this.delimiter}${value}`;
...
Stack.prototype.pop = function() {
  ...
  while(i-- && this.storage[i] !== this.delimiter) {
    ...
    this.storage = this.storage.slice(0, i);
  }
...
Stack.prototype.count = function() {
  ...
  for (var i = 0, result = 1; i < this.storage.length; i++) {
    if (this.storage[i] === this.delimiter) result++;
  }
